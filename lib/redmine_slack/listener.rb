...
class SlackListener < Redmine::Hook::Listener
	...
	def controller_issues_new_after_save(context={})
		...
		slack_users = []
		for mail in issue.recipients
			cf = UserCustomField.find_by_name("Slack name")
			cv = User.find_by_mail(mail).custom_value_for(cf)
			puts cv, cv.class
			next unless cv
	...
	def controller_issues_edit_after_save(context={})
		...
		return unless url and (Setting.plugin_redmine_slack[:post_updates] == '1' or ( Setting.plugin_redmine_slack[:post_closed] == '1' and issue.status.is_closed ) )
